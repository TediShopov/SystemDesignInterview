...
public class LineRendererSmoother : MonoBehaviour
{
    ...
    public void Smooth()
    {
        ...
        for (int i = 0; i < Curves.Length; i++)
        {
            ...
            Vector3 lastPosition = i == 0 ? this.Line.GetPosition(0) : this.Line.GetPosition(i - 1);
            Vector3 nextPosition = this.Line.GetPosition(i + 1);
    ...
    {
        this.Line.positionCount = Curves.Length * SmoothingSections;
        int index = 0;
        for (int i = 0; i < Curves.Length; i++)
        {
            ...
            {
                this.Line.SetPosition(index, segments[j]);
                index++;
            }
        }
    ...
    {
        if (Curves == null || Curves.Length != this.Line.positionCount - 1)
        {
            Curves = new BezierCurve[this.Line.positionCount - 1];
            for (int i = 0; i < Curves.Length; i++)
            {
